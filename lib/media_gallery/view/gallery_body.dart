...
class MediaGalleryBody extends StatefulWidget {
  /// Refer to [showImageViewerPager] for the arguments
  const MediaGalleryBody(
    this.imageProvider, {
    this.backgroundColor = _defaultBackgroundColor,
    this.closeButtonTooltip = _defaultCloseButtonTooltip,
    this.closeButtonColor = _defaultCloseButtonColor,
    super.key,
    this.immersive = true,
    this.onPageChanged,
  ...
  @override
  State<MediaGalleryBody> createState() => _MediaGalleryBodyState();
}

class _MediaGalleryBodyState extends State<MediaGalleryBody> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.swipeDismissible) {
      return Dismissible(
        ...
        key: const Key('__dismissible_media_gallery_body__'),
  ...
  void _handleDismissal() {
    if (widget.onViewerDismissed != null) {
      widget.onViewerDismissed?.call(_pageController.page?.round() ?? 0);
    }
...
class MediaModelImageProvider extends EasyImageProvider {
  ...
  ImageProvider<Object> imageBuilder(BuildContext context, int index) {
    ...
    return MediaCacheManager(
      fileUri,
      errorListener: (_) => Assets.images.missingImage,
    );
  }

  @override
  int get imageCount => mediaModels.length;
}
